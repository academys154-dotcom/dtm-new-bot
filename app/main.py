...
BTN_CODE_EDIT = "✏️ Kodni edit qilish"
...
RESULT_DETAIL_LIMIT = 0  # 0 = barcha savollar tahlili ko'rsatiladi
...
class AdminStates(StatesGroup):
    ...
    waiting_edit_code_old = State()
    waiting_edit_code_new = State()
...
def evaluate_answers(answer_key_tokens: list[str], user_answers_tokens: list[str]) -> dict:
    ...
    for index, correct_answer in enumerate(key, start=1):
        ...
        if is_ok:
            ...
            blank += 1
        if RESULT_DETAIL_LIMIT == 0 or index <= RESULT_DETAIL_LIMIT:
    ...
    if RESULT_DETAIL_LIMIT and total > RESULT_DETAIL_LIMIT:
...
class Database:
    ...
    def list_recent_codes(self, limit: int = 100) -> list[sqlite3.Row]:
        with closing(self.connect()) as conn:
            return conn.execute(
                """
                SELECT rc.*, e.title, e.subject, e.category, e.grade_level
                FROM redeem_codes rc
                JOIN exams e ON e.id = rc.exam_id
                ORDER BY rc.id DESC
                LIMIT ?
    ...
    def get_code_basic(self, code: str) -> Optional[sqlite3.Row]:
        with closing(self.connect()) as conn:
            return conn.execute(
                """
                SELECT rc.*, e.title, e.subject, e.category, e.grade_level
    ...
    def update_code_text(self, old_code: str, new_code: str) -> tuple[bool, str]:
        old_code = normalize_code(old_code)
        new_code = normalize_code(new_code)
        if not old_code or not new_code:
            return False, "Kod bo'sh bo'lmasin."
        if old_code == new_code:
            return False, "Yangi kod eski kod bilan bir xil."
        with closing(self.connect()) as conn:
            existing = conn.execute("SELECT id FROM redeem_codes WHERE code = ?", (new_code,)).fetchone()
            if existing:
                return False, "Bu yangi kod allaqachon mavjud."
            cur = conn.execute("UPDATE redeem_codes SET code = ? WHERE code = ?", (new_code, old_code))
            conn.commit()
            if cur.rowcount < 1:
                return False, "Eski kod topilmadi."
            return True, "Kod yangilandi."
...
def admin_menu() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text=BTN_CODE_EDIT), KeyboardButton(text=BTN_STATS)],
...
def format_code_line(row: sqlite3.Row) -> str:
    used = "ISHLATILGAN" if row["is_used"] else "bo'sh"
    owner = f" | user: {row['used_by_tg_user_id']}" if row["used_by_tg_user_id"] else ""
    return f"{row['code']} | ID {row['exam_id']} | {grade_label(row['grade_level'])} | {row['subject']} | {row['title']} | {used}{owner}"

def format_code_numbered_list(codes: list[sqlite3.Row]) -> str:
    return "\n".join(f"{index}) {format_code_line(row)}" for index, row in enumerate(codes, start=1))

def pick_code_from_number_or_text(text: str, codes: list[str]) -> str:
    raw = compact_spaces(text)
    if raw.isdigit():
        index = int(raw) - 1
...
async def send_long_text(message: Message, text: str, reply_markup: Optional[ReplyKeyboardMarkup] = None, chunk_size: int = 3800) -> None:
    chunks: list[str] = []
    current = ""
    ...
    for index, chunk in enumerate(chunks):
        ...
        await tracked_answer(message, chunk, reply_markup=markup)
...
async def admin_edit_code_new(message: Message, state: FSMContext) -> None:
    ...
    ok, msg = DB.update_code_text(old_code, new_code)
    await state.clear()
    await remember_numbered_options(state, ADMIN_MENU_OPTIONS)
    await tracked_answer(message, f"{'✅' if ok else '❌'} {msg}\nEski kod: <code>{escape_html_text(old_code)}</code>\nYangi kod: <code>{escape_html_text(new_code)}</code>\n\n" + admin_menu_text(), reply_markup=admin_menu())
...
async def user_answers_handler(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await send_long_text(message, result["result_text"], reply_markup=main_menu(message.from_user.id))
...
async def text_router(message: Message, state: FSMContext) -> None:
    ...
    if text == BTN_CODE_EDIT:
        codes = DB.list_recent_codes(limit=100)
        if not codes:
            await tracked_answer(message, "Hali yaratilgan kod yo'q.", reply_markup=admin_menu())
            return
        await state.update_data(edit_code_values=[row["code"] for row in codes])
        await state.set_state(AdminStates.waiting_edit_code_old)
        await tracked_answer(message, "Edit qilinadigan kodni tanlang. Ro'yxat raqami yoki eski kodni yuboring:\n" + format_code_numbered_list(codes), reply_markup=admin_menu())
        return
